...
auto getHelper2() {
    if constexpr (N == 0) {
        return L1{}.template get<M>();
    } else {
        return getHelper2<N - 1, M, Ls...>();
    }
}
...
Value execute(Func<NAME, Ts...> func, const std::vector<Value>& args, const auto& that) {
    if constexpr (N < decltype(func)::count) {
        ...
        if constexpr (is_any3) {
            v3 = args[2];
        } else {
            v3 = std::get<T3>(args[2]);
        }
