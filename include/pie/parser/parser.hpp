          ...
          public:
                ...
                while (not atEnd()) {
                    ...
                    if (not match(TokenKind::SEMI)) {
                        ...
                        if (t.kind == TokenKind::NAME) {
                            ...
                            if (t.text.length() > 1) {
                                msg += "Did you, perhaps, forget a ';' on the previous line?";
                            }
            ...
            expr::ExprPtr parseExpr(const int precedence = 0) {
                ...
                while (precedence < getPrecedence()) {
                    if constexpr (not PARSE_TYPE or CTX == Context::MAP) {
            ...
            expr::ExprPtr prefix(Token token) {
                ...
                case FLOAT:
                case INT:
                    return std::make_shared<expr::Num>(std::move(token).text);
                case BOOL:
                    return std::make_shared<expr::Bool>(token.text == "true" ? true : false);
                case STRING:
                    return std::make_shared<expr::String>(std::move(token).text);

                case NAME:
                    if constexpr (not PARSE_TYPE) {
                ...
                case NAMESPACE:
                    return nameSpace();
                case USE:
                    return std::make_shared<expr::Use>(parseExpr());
                ...
                case SCOPE_RESOLVE: {
                    ...
                    if (not right_name_ptr) {
                        error("Can only follow a '::' with a name/namespace access: " + right->stringify());
                    }
                ...
                case COLON:
                    return std::make_shared<expr::Type>(parseType());
                ...
                case L_BRACE: {
                        ...
                        for (size_t i{}; /* not atEnd(i) */; ++i) {
                            ...
                            if (check(COLON, i)) {
                                for (size_t a = i + 1; /* not atEnd(a) */; ++a) {
                                    if (check(COMMA, a)) {
                                        return true; // onto next element, it's a map
                                    }
                                    if (check(R_BRACE, a)) {
                                        return true; // closed the map, it's a map
                                    }
                                    if (check(COLON, a)) {
                                        return true; // this time the colon indicates a declaration {n1: n2: n3 = 4};
                                    }
                                    if (check(SEMI, a)) {
                                        return false; // proly a declaration, it's a scope..i think :c
                                    }

                                    if (check(L_BRACE, a)) {
                                        while (not check(R_BRACE, a)) {
                                            ++a;
                                        }
                                    }
                                    if (check(L_PAREN, a)) {
                                        while (not check(R_PAREN, a)) {
                                            ++a;
                                        }
                                    }
                                }
                            ...
                            if (check(COMMA, i)) {
                                return false; // if you find a comma first, it's a list {1, 2};
                            }
                            if (check(R_BRACE, i)) {
                                return false; // finding a `}` before finding `:` means it's a list with potentially one element
                            }
                            if (check(SEMI, i)) {
                                return false; // finding a `;` means it's a scope, and that was the end of an expression...
                            }

                            if (check(L_BRACE, i)) {
                                while (not check(R_BRACE, i)) {
                                    ++i;
                                }
                            }
                            if (check(L_PAREN, i)) {
                                while (not check(R_PAREN, i)) {
                                    ++i;
                                }
                            }
                        }
                    ...
                    if (map_expr) {
                        ...
                        consume(COLON);
                        std::vector<std::pair<expr::ExprPtr, expr::ExprPtr>> exprs = {
                            {
                                std::move(key),
                                parseExpr(),
                            },
            ...
            expr::ExprPtr infix(expr::ExprPtr left, Token token) {
                ...
                case NAME:
                    return infixName(std::move(left), std::move(token));
                ...
                case DOT: {
                    ...
                    if (not accessee_ptr) {
                        error("Can only follow a '.' with a name: " + accessee->stringify());
                    }
            ...
            type::TypePtr parseType() {
                ...
                if (match(ELLIPSIS)) {
                    if (not allow_variadic) {
                        error("Can't have a variadic of a variadic type!");
                    }
                ...
                if (match(L_BRACE)) { // list or map type
                    ...
                    if (match(R_BRACE)) {
                        return std::make_shared<type::ListType>(std::move(type1));
                    }
            ...
            expr::ExprPtr klass() {
                ...
                while (not match(R_BRACE)) {
                    ...
                    if (not ass) {
                        error("Can only have assignments in class definition!");
                    }
            ...
            expr::ExprPtr parseFoldExpr() {
                ...
                if (not ops.contains(op)) {
                    error("Folding over unknown operator: " + op);
                }
                if (ops[op]->type() != TokenKind::INFIX) {
                    error("Folding over non-infix operator: " + op);
                }

                if (match(ELLIPSIS)) {
                    // left unary fold (unseparated) | case 3
                    if (match(R_PAREN)) {
                        return std::make_shared<expr::UnaryFold>(std::move(pack), std::move(op), l2r);
                    }
                    ...
                    if (match(R_PAREN)) {
                        return std::make_shared<expr::SeparatedUnaryFold>(std::move(pack), std::move(rhs), std::move(op));
                    }
                ...
                if (match(op)) {
                    seperator = parseExpr(prec::HIGH_VALUE);
                }
                ...
                if (not match(R_PAREN)) { // while not closing the paren for the call
                    ...
                    do {
                        ...
                        if (auto ass = dynamic_cast<expr::Assignment*>(arg.get())) {
                            if (match(ELLIPSIS)) {
                                error("Cannot expand pack in named argument: " + ass->stringify());
                            }

                            if (not type::shouldReassign(ass->type)) {
                                error("Can't have type annotation for named arguments: " + ass->stringify());
                            }
            ...
            expr::ExprPtr prefixName(Token token) {
                if (ops.contains(token.text)) {
                    return parseOperator(std::move(token));
                }
            ...
            expr::ExprPtr infixName(expr::ExprPtr left, Token token) {
                if (ops.contains(token.text)) {
                    ...
                    case TokenKind::EXFIX: {
                        ...
                        if (token.text != op->name2) {
                            error("Open exfix operator found where closing one was expected!");
                        }
                    ...
                    case TokenKind::MIXFIX: {
                        ...
                        if (token.text != op->name) {
                            return left;
                        }
                        if (op->op_pos[0]) {
                            error("Operator '" + op->name + "' has to come before an expression!");
                        }
                    ...
                    default:
                        error("prefix operator used as [inf/suf]fix");
                    }
                }
            ...
            expr::ExprPtr parseOperator(Token token) {
                ...
                case TokenKind::MIXFIX: {
                    const auto& op = dynamic_cast<const expr::Operator*>(ops[token.text].get());
                    if (not op->op_pos[0]) {
                        error("Operator '" + token.text + "' has to come after an expression!");
                    }
            ...
            expr::ExprPtr fixOperator(Token token) {
                ...
                if (token.kind == EXFIX) {
                    return exfixOperator();
                }
                if (token.kind == MIXFIX) {
                ...
                if (match(L_PAREN)) {
                    ...
                    high = [shift, &low, this] {
                        if (shift > 0) {
                            return prec::higher(low, ops);
                        }
                        if (shift < 0) {
                            return std::exchange(low, prec::lower(low, ops));
                        }
                        return low;
                    }();
                    ...
                    name = low = high = consume(NAME).text;
                }
                ...
                if (not c) {
                    error("[pre/in/suf] fix operator has to be equal to a function!");
                }
                ...
                if (token.kind == PREFIX) {
                    if (c->params.size() != 1) {
                        error("Prefix operator must be assigned to a unary closure!");
                    }
                    ...
                    if (c->params.size() != 2) {
                        error("Infix operator must be assigned to a binary closure!");
                    }
            ...
            expr::ExprPtr exfixOperator() {
                ...
                expr::Closure* c = dynamic_cast<expr::Closure*>(func.get());
                if (not c) {
                    error("Exfix operator has to be equal to a function!");
                }
                if (c->params.size() != 1) {
                    error("Exfix operator must be assigned to a unary closure!");
                }
            ...
            expr::ExprPtr arbitraryOperator() {
                ...
                while (not check(ASSIGN)) {
                    if (match(SCOPE_RESOLVE)) {
                        op_pos.push_back(false), op_pos.push_back(false);
                    } else if (match(COLON)) {
                        op_pos.push_back(false);
                    } else {
                        op_pos.push_back(true);
                ...
                expr::Closure* c = dynamic_cast<expr::Closure*>(func.get());
                if (not c) {
                    error("Operators have to be equal to a function!");
                }
            ...
            [[nodiscard]] int getPrecedence() {
                ...
                // right associative
                case ASSIGN:
                    return prec::ASSIGNMENT_VALUE;

                // case COLON : return prec::SCOPE_RESOLUTION_VALUE;
                case SCOPE_RESOLVE:
                    return prec::SCOPE_RESOLUTION_VALUE;

                case DOT:
                    return prec::MEMBER_ACCESS_VALUE;
            ...
            void log(bool shift = false, bool begin = false) {
                ...
                const ptrdiff_t diff = begin ? 0 : dist < 5 ? dist
                                                            : 5;
