...
struct Visitor {
    ...
    void addOperators(Operators os) {
        ...
        for (auto& [name, fix] : os) {
            if (ops.contains(name)) {
                ...
                ops[std::move(name)] = std::move(fix);
            }
        }
    }

    Value operator()(const expr::Num* n) {
        if (const auto& var = getVar(n->stringify()); var) {
            return var->first;
        }
        ...
        if (n->num.find('.') != std::string::npos) {
            ...
            return std::stoll(n->num);
        }
    }

    Value operator()(const expr::Bool* b) {
        if (const auto& var = getVar(b->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::String* s) {
        if (const auto& var = getVar(s->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::Name* n) {
        ...
        if (const auto& var = getVar(n->name); var) {
            return var->first;
        }
        if (const auto var = checkThis(n->name); var) {
            return *var;
        }
    ...
    Value operator()(const expr::List* list) {
        if (const auto& var = getVar(list->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::UnaryFold* fold) {
        if (const auto& var = getVar(fold->stringify()); var) {
            return var->first;
        }
        ...
        if (not std::holds_alternative<PackList>(pack)) {
            error("Folding over a non-pack: " + stringify(pack));
        }

        auto& packlist = get<PackList>(pack);

        if (packlist->values.empty()) {
            error("Folding over an empty pack: " + fold->stringify());
        }
        if (packlist->values.size() == 1) {
            return packlist->values[0];
        }

        Value ret = fold->left_to_right ? packlist->values.front() : packlist->values.back(); // [packlist->values.size() - 2];
        const auto values = fold->left_to_right ? packlist->values | std::views::drop(1) | std::views::as_rvalue | std::ranges::to<std::vector<Value>>() : packlist->values | std::views::reverse | std::views::drop(1) | std::views::as_rvalue | std::ranges::to<std::vector<Value>>();
    ...
    Value accessAssign(const expr::Assignment* ass, expr::Access* acc) {
        ...
        if (not std::holds_alternative<Object>(left)) {
        ...
        if (found == obj.second->members.end()) {
            error("In assignment '" + ass->stringify() + "', Name '" + acc->name + "' doesn't exist in object: " + stringify(obj));
        }
    ...
    Value spaceAccessAssign(const expr::Assignment* ass, expr::SpaceAccess* sa) {
        ...
        if (not std::holds_alternative<NameSpace>(space)) {
            error("Can't apply scope-resolution-operator '::' on a non-namespace: " + sa->space->stringify());
        }
        ...
        if (found == ns.members->members.end()) {
            error("Name '" + sa->member + "' doesn't exist inside namesapce '" + sa->space->stringify() + '\'');
        }
    ...
    Value spaceAssign(const NameSpace& ns1, const NameSpace& ns2) {
        for (const auto& [name, type, value] : ns2.members->members) {
            ...
            // same here
                ...
                ns1.members->members.push_back({name, type, value});
            }
        }
    ...
    Value nameAssign(const expr::Assignment* ass, const expr::Name* name) {
        ...
        if (change) {
            if (not changeVar(name->name, value)) {
                error();
            }
        } else {
    ...
    Value operator()(const expr::Assignment* ass) {
        ...
        if (auto* acc = dynamic_cast<expr::Access*>(ass->lhs.get())) {
            return accessAssign(ass, acc);
        }

        // ns::x = 1;
        if (auto* sa = dynamic_cast<expr::SpaceAccess*>(ass->lhs.get())) {
            return spaceAccessAssign(ass, sa);
        }

        // name = 1;
        if (auto name = dynamic_cast<expr::Name*>(ass->lhs.get())) {
            return nameAssign(ass, name);
        }
    ...
    Value operator()(const expr::Class* cls) {
        if (const auto& var = getVar(cls->stringify()); var) {
            return var->first;
        }
    ...
    Value objectAccess(const Object& obj, const std::string& name) {
        ...
        if (found == obj.second->members.end()) {
            error("Name '" + name + "' doesn't exist in object '" + /*acc->var->*/ stringify(obj) + '\'');
        }
    ...
    Value operator()(const expr::Access* acc) {
        ...
        const auto& left = std::visit(*this, acc->var->variant());
        if (std::holds_alternative<Object>(left)) {
            return objectAccess(std::get<Object>(left), acc->name);
        }
    ...
    Value operator()(const expr::Namespace* ns) {
        if (const auto& var = getVar(ns->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::Use* use) {
        ...
        if (not std::holds_alternative<NameSpace>(ns)) {
            error("Can't apply keyword 'use' on a non-namespace: " + use->ns->stringify());
        }
    ...
    Value operator()(const expr::SpaceAccess* sa) {
        ...
        if (not sa->space) {
            if (const auto& var = globalLookup(sa->member); var) {
                return var->first;
            } else {
                error("Name '" + sa->member + "' not found in global namespace!");
            }
        }
    ...
    bool match(const Value& value, const expr::Match::Case::Pattern& pattern) {
        ...
        if (obj.second->members.size() != obj.second->members.size()) {
            error("idek what error message this should be..!");
        }

        for (const auto& [member, pat] : std::views::zip(get<Object>(value).second->members, patterns)) {
            if (not match(get<Value>(member), *pat)) {
                return false;
            }
        }
    ...
    Value operator()(const expr::Match* m) {
        ...
        for (const auto& kase : m->cases) {
            ...
            if (match(value, *kase.pattern)) {
                ...
                if (guard) {
                    return std::visit(*this, kase.body->variant());
                }
            }
        }
    ...
    Value operator()(const expr::Loop* loop) {
        ...
        const auto classify = [](const Value& v) {
            if (std::holds_alternative<ssize_t>(v)) {
                return Type::INT;
            }
            ...
            if (std::holds_alternative<PackList>(v)) {
                return Type::PACK;
            }
        ...
        if (loop->kind) {
            ...
            case Type::INT: {
                ...
                if (limit <= 0) {
                    if (not loop->els) {
                        error("Loop which didn't run doesn't have else branch: " + loop->stringify());
                    }
            ...
            case Type::LIST: {
                ...
                if (loop->var) {
                    ...
                    for ([[maybe_unused]] const auto& _ : list.elts->values) {
                        continued = false;
            ...
            case Type::PACK: {
                ...
                if (loop->var) {
                    ...
                    for ([[maybe_unused]] const auto& _ : pack->values) {
                        continued = false;
            ...
            case Type::OBJECT: {
                ...
                if (
                    not hasNext_func.type.params.empty() or hasNext_func.type.ret->text() != "Bool" or not next_func.type.params.empty()) {
                    error("Object in loop: " + loop->stringify() + " doesn't follow the iterator protocol!");
                }
    ...
    Value operator()(const expr::Break* brake) {
        broken = true;
        if (brake->expr) {
            return std::visit(*this, brake->expr->variant());
        }
    ...
    Value operator()(const expr::Continue* cont) {
        continued = true;
        if (cont->expr) {
            return std::visit(*this, cont->expr->variant());
        }
    ...
    Value operator()(const expr::Grouping* g) {
        if (const auto& var = getVar(g->stringify()); var) {
            return var->first;
        }
        ...
        for (const auto& func : funcs) {
            ...
            for (const auto& param_type : closure->type.params) {
                if (param_type->text() == "Syntax") {
                    error("Cannot have paramater of 'Syntax' in an overload set!");
                }
            }
        }
    ...
    static expr::Closure* resolveOverloadSet(
        ...
        if (set.size() != 1) {
            error("Could not resolve overload set for operator: " + name);
        }
    ...
    Value operator()(const expr::UnaryOp* up) {
        if (const auto& var = getVar(up->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::BinOp* bp) {
        if (const auto& var = getVar(bp->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::PostOp* pp) {
        if (const auto& var = getVar(pp->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::CircumOp* cp) {
        if (const auto& var = getVar(cp->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::OpCall* oc) {
        if (const auto& var = getVar(oc->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::Call* call) {
        if (const auto& var = getVar(call->stringify()); var) {
            return var->first;
        }
        ...
        if (std::holds_alternative<std::string>(var)) {    // that dumb lol. but now it works
            ...
            if (isBuiltin(name)) {
                return evaluateBuiltin(std::move(args), std::move(expand_at), call->named_args, name);
            }
        }
        ...
        if (std::holds_alternative<type::TypePtr>(var)) {
    ...
    Value closureCall(
        ...
        if (not is_variadic and args_size + call->named_args.size() > func.params.size()) {
            error("Too many arguments passed to function: " + call->stringify());
        }
        ...
        for (const auto& [name, expr] : call->named_args) {
            ...
            if (not type) {
                error(); //* should never happen anyway
            }
        ...
        if (is_variadic) {
            ...
            if (variadic_size == 0) {
                ...
                // }});
                sg.addEnv({{pos_params[variadic_index].first,
                            {makePack(), pos_params[variadic_index].second}}});
    ...
    void captureEnvForClosure(const expr::Closure& c) {
        for (size_t i = env.size() - 1; /* i >= 0 */; --i) {
            ...
            if (i == 0) {
                return; // unsigned nums can never be less than zero
            }
        }
    }
    ...
    Value partialApplication(
        ...
        for (const auto& [name, expr] : call->named_args) {
            ...
            if (not type) {
                util::error(); // should never happen anyway
            }
                ...
                if (const auto& type_of_value = typeOf(value); not(*type >= *type_of_value)) {
                    error<except::TypeMismatch>(
                        "Type mis-match! Parameter '" + name + "' "
                                                               "expected type: " +
                        type->text() + ", got: " + type_of_value->text());
                }
            }
    ...
    Value constructorCall(const expr::Call* call, Value var) {
        ...
        if (not type::isClass(type)) {
            if (not call->args.empty()) {
                error("Can't pass arguments to non-class types: " + call->stringify());
            }
            if (type::isFunction(type)) {
                error("Can't default-construct a function type: " + call->stringify());
            }
            ...
            if (auto onion = type::isUnion(type)) {
                if (onion->types.empty()) {
                    error("Cannot default construct type 'Never', or 'union { }': " + call->stringify());
                }
    ...
    Value operator()(const expr::Closure* c) {
        if (const auto& var = getVar(c->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::Block* block) {
        if (const auto& var = getVar(block->stringify()); var) {
            return var->first;
        }
    ...
    Value operator()(const expr::Fix* fix) {
        if (const auto& var = getVar(fix->stringify()); var) {
            return var->first;
        }
    ...
    Value evaluateBuiltin(
        ...
        const auto functions = stdx::make_indexed_tuple<KeyFor>(
            ...
            MapEntry<
                ...
                Func<"input_int",
                     decltype([](const auto&) {
                         std::string out;
                         std::getline(std::cin, out);
                         if (not std::ranges::all_of(out, isdigit)) {
                             error("'__builtin_input_int' recieved a non-int \"" + out + "\"!");
                         }
            ...
            MapEntry<
                S<"pow">,
                Func<"pow",
                     decltype([](const auto& a, const auto& b, const auto&) -> std::common_type_t<decltype(a), decltype(b)> { return std::pow(a, b); }),
                     TypeList<ssize_t, ssize_t>,
                     TypeList<ssize_t, double>,
                     TypeList<double, ssize_t>,
                     TypeList<double, double>>>{},
        ...
        const auto arity_check = [&name, &args](const size_t arity) {
            if (args.size() != arity) {
                error("Wrong arity with call to \"__builtin_" + name + "\"");
            }
        };
        ...
        */
        ...
        if (name == "concat") {
            if (args.size() < 2) {
                error("'concat' requires at least 2 argument passed!");
            }

            std::string s;
            for (const auto& arg : args) {
                const Value& v = std::visit(*this, arg->variant());
                if (not std::holds_alternative<std::string>(v)) {
                    error("'concat' only accepts strings as arguments: " + stringify(v));
                }
        ...
        if (name == "neg" or name == "not" or name == "reset") {
            arity_check(1); // just for now..
        }
        ...
        if (name == "reset") {
            ...
            if (const auto& v = getVar(s); not v) {
                error("Reseting an unset value: " + s);
            } else {
                removeVar(s);
            }
        ...
        if (name == "eval") {
            return execute<1>(stdx::get<S<"eval">>(functions).value, {value1}, this);
        }
        if (name == "neg") {
            return execute<1>(stdx::get<S<"neg">>(functions).value, {value1}, this);
        }
        if (name == "not") {
            return execute<1>(stdx::get<S<"not">>(functions).value, {value1}, this);
        }
        if (name == "len") {
            return execute<1>(stdx::get<S<"len">>(functions).value, {value1}, this);
        }
        if (name == "pop") {
            return execute<1>(stdx::get<S<"pop">>(functions).value, {value1}, this);
        }
        if (name == "to_int") {
            return execute<1>(stdx::get<S<"to_int">>(functions).value, {value1}, this);
        }
        if (name == "to_double") {
            return execute<1>(stdx::get<S<"to_double">>(functions).value, {value1}, this);
        }
        if (name == "to_string") {
            return execute<1>(stdx::get<S<"to_string">>(functions).value, {value1}, this);
        }
        ...
        if (std::ranges::find(eager, name) != eager.end()) {
            ...
            if (name == "get") {
                return execute<2>(stdx::get<S<"get">>(functions).value, {value1, value2}, this);
            }
            if (name == "push") {
                return execute<2>(stdx::get<S<"push">>(functions).value, {value1, value2}, this);
            }

            if (name == "add") {
                return execute<2>(stdx::get<S<"add">>(functions).value, {value1, value2}, this);
            }
            if (name == "sub") {
                return execute<2>(stdx::get<S<"sub">>(functions).value, {value1, value2}, this);
            }
            if (name == "mul") {
                return execute<2>(stdx::get<S<"mul">>(functions).value, {value1, value2}, this);
            }
            if (name == "div") {
                return execute<2>(stdx::get<S<"div">>(functions).value, {value1, value2}, this);
            }
            if (name == "mod") {
                return execute<2>(stdx::get<S<"mod">>(functions).value, {value1, value2}, this);
            }
            if (name == "pow") {
                return execute<2>(stdx::get<S<"pow">>(functions).value, {value1, value2}, this);
            }
            if (name == "gt") {
                return execute<2>(stdx::get<S<"gt">>(functions).value, {value1, value2}, this);
            }
            if (name == "geq") {
                return execute<2>(stdx::get<S<"geq">>(functions).value, {value1, value2}, this);
            }
            if (name == "eq") {
                return execute<2>(stdx::get<S<"eq">>(functions).value, {value1, value2}, this);
            }
            if (name == "leq") {
                return execute<2>(stdx::get<S<"leq">>(functions).value, {value1, value2}, this);
            }
            if (name == "lt") {
                return execute<2>(stdx::get<S<"lt">>(functions).value, {value1, value2}, this);
            }
        ...
        if (name == "and") {
            ...
            if (not std::holds_alternative<bool>(value1)) {
                return value1; // return first falsy value
            }
            if (not get<bool>(value1)) {
                return value1; // first falsey value
            }
        ...
        if (name == "or") {
            ...
            if (get<bool>(value1)) {
                return value1; // first truthy value
            }
            return std::visit(*this, args[1]->variant()); // last falsey value
        }
        ...
        if (name == "str_slice") {
            ...
            if (
                ...
                error(
                    "Invalid argument: __builtin_str_slice(" + args[0]->stringify() + ", " + args[1]->stringify() + ", " + args[2]->stringify() + ", " + args[3]->stringify() + ", ");
            }
    ...
    Value builtinPrint(
        ...
        const std::unordered_map<std::string, expr::ExprPtr>& named_args) {
        if (args.empty()) {
            error("'print' requires at least 1 positional argument passed!");
        }
    ...
    type::TypePtr validateType(const type::TypePtr& type) {
        ...
        if (const auto var_type = dynamic_cast<type::ExprType*>(type.get())) {
            ...
            if (variadic_type->type->text() == "Syntax") {
                error("Variadics of 'Syntax' is not allowed!");
            }
            ...
            // todo: allow this in the future
            if (list_type->type->text() == "Syntax") {
                error("List of 'Syntax' is not allowed!");
            }

            if (type::isVariadic(list_type->type)) {
                error("Lists of variadics types are not allowed!");
            }
            ...
            // todo: allow this in the future
            if (map_type->key_type->text() == "Syntax") {
                error("Map of 'Syntax' is not allowed!");
            }
            if (map_type->val_type->text() == "Syntax") {
                error("Map of 'Syntax' is not allowed!");
            }

            if (type::isVariadic(map_type->key_type)) {
                error("Map of variadics types are not allowed!");
            }
            if (type::isVariadic(map_type->val_type)) {
                error("Map of variadics types are not allowed!");
            }
    ...
    type::TypePtr typeOf(const Value& value) {
        if (std::holds_alternative<expr::Node>(value)) {
            return type::builtins::Syntax();
        }
        if (std::holds_alternative<ssize_t>(value)) {
            return type::builtins::Int();
        }
        ...
        if (std::holds_alternative<PackList>(value)) {
            ...
            if (values.empty()) {
                return std::make_shared<type::VariadicType>(type::builtins::_());
            }
            ...
            if (same) {
                return type::VariadicOf(std::move(values)[0]);
            }
        ...
        if (std::holds_alternative<ListValue>(value)) {
            ...
            if (values.empty()) {
                return type::ListOf(type::builtins::_());
            }
            ...
            if (same) {
                return type::ListOf(std::move(values)[0]);
            }
        ...
        if (std::holds_alternative<MapValue>(value)) {
            ...
            if (values.empty()) {
                return std::make_shared<type::MapType>(type::builtins::_(), type::builtins::_());
            }
        ...
        if (std::holds_alternative<NameSpace>(value)) {
            return std::make_shared<type::SpaceType>();
        }
