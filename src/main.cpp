    ...
    for (;;) {
        try {
            ...
            if (print_preprocessed) {
                std::println(std::clog, "{}", processed_line);
            }

            Tokens v = lex::lex(std::move(processed_line));
            if (print_tokens) {
                std::println(std::clog, "{}", v);
            }
            ...
            if (run and (print_parsed or print_preprocessed or print_tokens)) {
                puts("Output:\n");
            }
            ...
            if (run) {
                ...
                if (not exprs.empty()) {
                    Value value;
                    for (auto&& expr : exprs) {
                        value = std::visit(visitor, std::move(expr)->variant());
                    }
...
int main(int argc, char* argv[]) {
    ...
    for (; argc > 1; --argc, ++argv) {
        if (argv[1] == "-token"sv) {
            ...
            print_parsed = true;
        } else if (argv[1] == "-pre"sv) {
            print_preprocessed = true;
        } else if (argv[1] == "-opt"sv) {
            print_opt = true;
        } else if (argv[1] == "-run"sv) {
            run = false;
        } else if (argv[1] == "-repl"sv) {
            repl = true;
        } else {
            fname = argv[1];
        }
